...
public class ListUDG {
     ...
     * @param start 开始顶点
     * @param end   结束顶点
     * @return 获取边<start, end>的权值，若start和end不是连通的，则返回无穷大。
     */
    ...
    /**
     * Dijkstra最短路径。
     * 统计图中"顶点s"到其它各个顶点的最短路径。
     *
     * @param s   起始顶点(start vertex)。即计算"顶点s"到其它顶点的最短路径。
     * @param prev 前驱顶点数组。prev[i]的值是"顶点s"到"顶点i"的最短路径所经历的全部顶点中，位于"顶点i"之前的那个顶点，也可以说是顶点s的父顶点
     * @param dist 长度数组。dist[i]是"顶点s"到"顶点i"的最短路径的长度。
     */
    public void dijkstra(int s, int[] prev, int[] dist) {
        ...
        for (int i = 0; i < mVexs.length; i++) {
            ...
            // 顶点i的前驱顶点为0。
            prev[i] = 0;
            // 顶点i的最短路径为"顶点s"到"顶点i"的权。
            dist[i] = getWeight(s, i);
        }
        ...
        for (int i = 0; i < mVexs.length; i++) {
            System.out.printf(" shortest(%c, %c)=%d\n", mVexs[s].data, mVexs[i].data, dist[i]);
        }
    }
